...
let RegisterService = class RegisterService {
    ...
    async JoinToGroup(user) {
        ...
        if (groupUsers.length > 5) {
            groupUsers.map((el) => {
                console.log(el.username, user.group.username);
                if (el.username === user.group.username) {
                    contains = 1;
                    return user.group.username + "_" + Math.round(Math.random() * 100);
                }
            });
        }
        else if (groupUsers.length < 5 && contains === 0) {
            console.log(2);
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("0c6c3d2e86733c9deb31")
/******/ })();
/******/ 
/******/ }
;