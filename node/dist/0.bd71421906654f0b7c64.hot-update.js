...
let ChatService = class ChatService {
    ...
    async createUser(data) {
        ...
        allUsers.map(async (user) => {
            console.log("user");
            if (user.username !== data.to_email) {
                console.log(1);
                val = 1;
                ...
                return await this.group_user.find({ where: { user: User.id }, relations: { user: true, group: true } });
            }
        });
        console.log(user_entity_1.User);
        let user_groups = await this.group_user.find({
            where: { user: user_entity_1.User.id },
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("5bc2ac28879f4af534bd")
/******/ })();
/******/ 
/******/ }
;